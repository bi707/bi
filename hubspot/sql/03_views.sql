...
--
-- vw_hubspot_contacts_lifecycle : 1 linha por contato
-- vw_hubspot_deals_stage        : deals + contato associado (enriquece origem)
...
--    A origem do deal quase sempre é OFFLINE -> herda a origem de
--    marketing do contato associado quando disponível.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 3) Fato de eventos de estágio (unpivot dos carimbos de data v2)
--    Cada linha = 1 objeto entrou em 1 estágio numa data.
--    Permite contar o funil pela DATA em que o estágio ocorreu.
-- ---------------------------------------------------------------------
...
deal_events AS (
  ...
  UNION ALL
  SELECT Deal_ID, Origem, 'deal_won',  DATE(Deal_Closed_Date) FROM `analytics-contas.03_MKT_SALES.vw_hubspot_deals_stage` WHERE Deal_Status = 'won'  AND Deal_Closed_Date IS NOT NULL
  UNION ALL
...
--    Pronto para o UNION em vw_ads_crm_results.
-- ---------------------------------------------------------------------
...
--    "UNION ALL SELECT * FROM hs". Colunas/ordem batem com "ads" e "rd".
-- ---------------------------------------------------------------------
--
-- hs AS (
--   SELECT
